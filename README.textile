...
h2. Option to fail the build based on the set threshold limit on benchmarks (New feature added)

This option is newly added to this gem in order to fail the build if one of the top 15 slowest test's benchmark shows a result of exceeding the maximum time(threshold) that a test can take to run.
